...
import type { BrandCode, PerformanceRow } from "@marketingkpi/core"

interface PerformanceAnalysisPageProps {
  brand: BrandCode
}

export default function PerformanceAnalysisPage({ brand }: PerformanceAnalysisPageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true)
      const rows = await fetchPerformanceRows(brand)
  ...
  const filteredData = data.filter(r => {
    if (r.isSummary) return true; // 총계는 항상 유지
